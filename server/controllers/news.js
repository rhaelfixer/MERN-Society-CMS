...
const sendNewsNotification = require("../middlewares/sendNewsNotification");
const updateNewsNotification = require("../middlewares/updateNewsNotification");
const sendTestEmail = require("../middlewares/sendTestEmail");

const SEND_NEWS_NOTIFICATION = process.env.NODE_ENV === "production" ? sendNewsNotification : sendTestEmail;
const UPDATE_NEWS_NOTIFICATION = process.env.NODE_ENV === "production" ? updateNewsNotification : sendTestEmail;
...
router.post("/create", authenticateToken, isAdmin, upload.single("image"), async (req, res, next) => {
  try {
    ...
    // Validate the News object before saving
    try {
      await newNews.validate();
    ...
    if (req.file) {
      ...
      if (cloudinaryResponse.secure_url) {
        ...
        // Set the image URL in the News object
    ...
    // Send the News notification asynchronously
    SEND_NEWS_NOTIFICATION(req, res, next)
      .then(() => {
...
router.get("/preview/:id", async (req, res, next) => {
  const {id} = req.params;
  try {
    // Find the News in the database by its id
...
router.put("/update/:id", authenticateToken, isAdmin, upload.single("image"), async (req, res, next) => {
  ...
  try {
    ...
    // Send the News notification asynchronously
    UPDATE_NEWS_NOTIFICATION(req, res, next)
      .then(() => {
        console.log("Updated News Notification sent successfully!");
      })
      .catch(error => {
        console.error("Error sending Update News Notification:", error);
      });
...
router.delete("/delete/:id", authenticateToken, isAdmin, async (req, res, next) => {
  ...
  try {
    ...
    if (news.image) {
      const publicIdMatch = news.image.match(/\/v\d+\/(.+)\./);
      ...
      if (publicId) {
        ...
        console.log("Invalid image URL:", news.image);
      }
    }
